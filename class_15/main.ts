...
let sumAdd = (num1:number, num2:number) => {
    return num1 + num2
}
let ans:number = sum(8,8)
console.log(ans);
// console.log(sum(8,8));
...
function checkEvenOdd(numEvenOdd: number) {
    if (numEvenOdd % 2 === 0 ) {
...
let arrayOperations =()=>{
    let arr:number[] =   [4,8,6,12,28] // 8 12 28 36
    arr.push(36);
    arr.shift();
    arr.splice(1,1)
    console.log(arr);
...
function multiplies(arr0:number[]) {
    for (let index = 0; index < arr0.length; index++){ 
        arr0[index] *= 2;   
    }
    return arr0
}
let myArray:number[] = [2,4,8,5]
...
function largestNumber(...arrA:number[]) {
    let largest = arrA[0]
...
function positive(numbers:number[]):number {
    let sum = 0
...
function filterStringsByLength(strings: string[]): string[] {
